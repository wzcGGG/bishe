...
def multiSimulationP(simulationCount, modelConfig, days, debug, modelName,realdata, outputDir="outputs"):
    """
        run multiple simulations and return the location where the infection occured, the total number infected, and the max infected
        选择base_p
    """
    ...
    for i in range(simulationCount):
        result = simpleCheck(modelConfig, days=days, visuals=False, debug=debug, modelName=modelName+"_"+str(i))
        I1 = np.array(result['infected Asymptomatic'])
        I2 = np.array(result['infected Asymptomatic Fixed'])
        I3 = np.array(result['infected Symptomatic Mild'])
        I4 = np.array(result['infected Symptomatic Severe'])
        I = (I1 + I2 + I3 + I4) * 0.7826
        simuI = np.around(I)
        abs_error = np.sum(np.abs(realdata - simuI))
        if abs_error < error:
            error = abs_error
            bestI = simuI
            tu.realandI(days , bestI, name = str(i))
            print("发现更小")
    return bestI, error

def multiSimulationSIR(simulationCount, modelConfig, days, debug, modelName, outputDir="outputs"):
    """
        run multiple simulations and return the location where the infection occured, the total number infected, and the max infected
        返回SIR平均值
    """
    S_tatal = np.zeros([1, days])
    I_tatal = np.zeros([1, days])
    R_tatal = np.zeros([1, days])
    for i in range(simulationCount):
        result = simpleCheck(modelConfig, days=days, visuals=False, debug=debug, modelName=modelName+"_"+str(i))
        I1 = np.array(result['infected Asymptomatic'])
        I2 = np.array(result['infected Asymptomatic Fixed'])
        ...
        I4 = np.array(result['infected Symptomatic Severe'])
        I = (I1 + I2 + I3 + I4) * 0.7826
        I_tatal = np.row_stack((I_tatal, I))
        S = np.array(result['susceptible'])
        R = np.array(result['recovered'])
        S_tatal = np.row_stack((S_tatal, S))
        R_tatal = np.row_stack((R_tatal, R))
    S_tatal = S_tatal[1:, :]
    I_tatal = I_tatal[1:, :]
    R_tatal = R_tatal[1:, :]
    meanS = np.average(S_tatal, axis=0)
    meanI = np.average(I_tatal, axis=0)
    meanR = np.average(R_tatal, axis=0)
    return meanS,meanI,meanR
...
class AgentBasedModel:
    ...
    def initializeAgentsLocation(self):
        ...
        if self._debug:
            print(f"the agents we initialized using values (特定初始, 校外叶子节点, 校内宿舍) --> {counter}")
        spCounter = [0,0,0]
    ...
    def supersharp1(self,):
        ...
        while i < n:
            ...
            i = i + 1
        # for i in range(0, n):
        #     self.changeStateDict(tepS[i], self.agents[tepS[i]].state, "exposed")
        for i in range(0, 16):
            self.changeStateDict(tepS[i], self.agents[tepS[i]].state, "infected Asymptomatic Fixed")
            # print("序号:" + str(i) + "*****" + "ID:" + str(tepS[i]) + "改变")
        for i in range(16, n):
            self.changeStateDict(tepS[i], self.agents[tepS[i]].state, "infected Asymptomatic")
        print("13天超级传播事件发生.")
        pass

    def supersharp2(self,):
        S = []
        tepS = []
        for agentId, agent in self.agents.items():
            if agent.state == "susceptible":
    ...
    def updateSteps(self, step = 1):
        for _ in range(step):
            ...
            if self.dateDescriptor != "W" or self.dateDescriptor!="LS":
                ...
                if self.quarantine_intervention and self.time%self.quarantineInterval == self.config["Quarantine"]["offset"] and self.date !=42 and self.date != 84:
    ...
    def updateAgent(self):
        ...
        for agentId, agent in self.agents.items():
            ...
            if loc[0] != loc[1]:
                # if the agent is coming back to the network from the offcampus node 回到学校
